...
#include <stdio.h>
#include <math.h>
int a(double x, double y) //вводим функцию с логическим выражением, обозначающим заданную область
{
  return (((x*x+y*y<=1) && (x*x+y*y>=0.25) && (x>=0)&(y>=0)) || ((x*x+y*y<=1) && (x<=0)&&(y>=0)));//ф-ция возвращает лог. выражение
}
int main(void)
{
    double x, y;
    ...
    scanf("%lf",&y);//ввод переменной у
    if (a(x,y)==1) //если функция от а правда
    {
